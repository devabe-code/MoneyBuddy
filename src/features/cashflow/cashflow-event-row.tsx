...
export function CashflowEventRow({
  event,
  showDate = false,
}: {
  event: CashflowEvent;
    ...
    <View accessibilityLabel={`${event.label}, ${event.amount}`} style={styles.row}>
      {showDate && event.month && event.day ? (
        <View style={styles.dateTile}>
          <AppText style={styles.month} tone="primary" variant="eyebrow">
            {event.month}
          </AppText>
          <AppText style={styles.day} variant="sectionTitle">
            {event.day}
          </AppText>
        </View>
      ) : null}
      ...
      <AppText style={styles.amount} tone={event.positive ? 'primary' : 'default'} variant="label">
        {event.amount}
      </AppText>
    </View>
  );
