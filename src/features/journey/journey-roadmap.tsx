...
export function JourneyRoadmap({
  ...
  return (
    <SurfaceCard>
      <AppText style={styles.roadmapTitle} variant="title">
        {title}
      </AppText>
        ...
        >
          <View accessibilityElementsHidden style={styles.rail}>
            <View style={[styles.marker, item.complete && styles.markerComplete]}>
              <Ionicons
                color={item.complete ? tokens.color.onPrimary : tokens.color.primary}
                name={item.complete ? 'checkmark' : item.icon}
                size={16}
              />
            </View>
            {index < milestones.length - 1 ? (
              <View style={[styles.line, item.complete && styles.lineComplete]} />
            ) : null}
          </View>
          <View style={styles.stepContent}>
            <AppText tone="primary" variant="eyebrow">
              {item.date}
            </AppText>
