...
describe('SyntheticSavingsGoalRepository', () => {
  ...
  it('allows freshness to be selected without changing feature code', async () => {
    await expect(new SyntheticSavingsGoalRepository([], 'fresh').list()).resolves.toMatchObject({
      freshness: 'fresh',
    });
  });
  ...
  it('provides deterministic cached and failure scenarios', async () => {
    await expect(
      new SyntheticSavingsGoalRepository(SYNTHETIC_SAVINGS_GOALS, { kind: 'offline' }).list(),
