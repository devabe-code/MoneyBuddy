...
import {
  defaultSavingsGoalProgressPolicy,
  type SavingsGoalProgressPolicy,
} from './savings-goal-progress';

function goal(savedMinor: bigint, targetMinor = 1_200_000n) {
  return defineSavingsGoal({
    currency: 'USD',
    id: 'synthetic-goal',
    kind: 'custom',
    name: 'Synthetic goal',
...
describe('SavingsGoalProgressPolicy', () => {
  ...
  it('calculates progress and remaining value with integer arithmetic', () => {
    expect(policy.summarize(goal(744_000n))).toEqual({
      progressPercent: 62,
      remainingMinor: 456_000n,
    });
  });
  ...
  it('caps already-funded goals without creating a negative remainder', () => {
    expect(policy.summarize(goal(1_300_000n))).toEqual({
      progressPercent: 100,
      remainingMinor: 0n,
    });
  });
